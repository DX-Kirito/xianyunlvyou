<template>
  <div class="flight-item">
    ...
    <div v-for="(item,index) in flights.seat_infos" :key="index" v-if="isShow" class="flight-recommend">
      <!-- 隐藏的座位信息列表 -->
...
export default {
  ...
  computed: {
    useTime () {
      const dep = new Date(this.flights.dep_datetime).getTime()
      const des = new Date(this.flights.arr_datetime).getTime()
      let useTime = des - dep
      if (useTime < 0) {
        useTime = (des - dep) + 24 * 60 * 60 * 1000
      }
      const hours = Math.floor(useTime / 1000 / 60 / 60)
      const minutes = (useTime / 1000 / 60) % 60
